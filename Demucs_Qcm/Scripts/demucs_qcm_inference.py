...
Usage:
    python demucs_qcm_inference.py \
        ...
        --question "Which source dominates?" \
        --choices '{"A": "Vocals", "B": "Drums"}'
"""
...
def _load_demucs_model(device: str = "cpu"):
    """Load the htdemucs model, downloading on first use if needed.
    
    Returns the model object ready for separation.
    """
    try:
...
def _separate_audio(model, audio_path: str, device: str = "cpu") -> Dict[str, np.ndarray]:
    ...
    Returns dict mapping stem name to mono numpy array.
    """
    import torch
    import torchaudio
    
    # Load audio at 44100 Hz (Demucs native sample rate)
    ...
    # Apply separation
    try:
        # demucs 4.x API: Pretrained.separate()
        if hasattr(model, 'separate'):
            stems_dict = model.separate(waveform)
            # stems_dict is already {name: tensor}
            result = {}
        ...
        # separated shape: (1, num_sources, channels, samples)
...
def separate_and_analyze(
    ...
    """Separate audio into stems and return RMS energy per stem.

    Returns:
        Dict mapping stem name ('vocals', 'drums', 'bass', 'other') to RMS energy.
    """
    model = _load_demucs_model(device)
    stems = _separate_audio(model, audio_path, device)
    
    # Compute RMS energy per stem
    rms_energies = {}
...
def _keyword_to_stems(keyword: str) -> list:
    """Map a keyword to relevant stem names."""
...
def _compute_choice_score(choice_text: str, rms_energies: Dict[str, float]) -> float:
    """Compute a relevance score for a choice based on stem energies."""
    stems = _keyword_to_stems(choice_text)
    if not stems:
        # If no keyword match, use total energy as fallback
...
def _detect_pattern(
    ...
    rms_energies: Dict[str, float],
) -> Optional[Dict]:
    """Try to detect known QCM patterns and answer directly."""
    q_lower = question.lower()
    choice_values = {k: v.lower() for k, v in choices.items()}
    
    # Pattern 1: "Which is most prominent/dominant/loudest?"
    if any(w in q_lower for w in ["dominant", "prominent", "loudest", "principal", "plus fort"]):
        ...
        if best_choice:
            return {
                ...
                "answer": best_choice,
                "scores": {
                    k: _compute_choice_score(v, rms_energies)
                    for k, v in choices.items()
                },
            }
    
    # Pattern 2: Speech vs Music
    if any(w in q_lower for w in ["speech", "music", "parole", "musique", "vocal", "instrumental"]):
        vocal_energy = rms_energies.get("vocals", 0.0)
        music_energy = (
            rms_energies.get("drums", 0.0)
            + rms_energies.get("bass", 0.0)
            + rms_energies.get("other", 0.0)
        )
        for key, text in choice_values.items():
            if any(w in text for w in ["speech", "parole", "voice", "vocal"]):
                if vocal_energy > music_energy:
                    return {
                        "pattern": "speech_vs_music",
                        "answer": key,
                        "scores": {"speech": vocal_energy, "music": music_energy},
                    }
    ...
    # Pattern 3: Presence detection (is there X?)
...
def answer_qcm(
    ...
    """Answer a QCM based on stem energies.

    Args:
        stems: Dict mapping stem name to RMS energy.
        question: The QCM question text.
        choices: Dict mapping choice key (A, B, ...) to choice text.
    ...
    # Fallback: score each choice by keyword-to-stem mapping
    choice_scores = {}
    for key, text in choices.items():
        choice_scores[key] = _compute_choice_score(text, stems)
    
    # If all scores are equal (no keyword matches), use total energy ranking
...
def main():
    ...
    parser.add_argument("--question", required=True, help="QCM question text")
    parser.add_argument("--choices", required=True, help='JSON dict of choices, e.g. {"A": "Vocals", "B": "Drums"}')
    ...
    try:
        ...
        print(json.dumps({"error": f"Invalid choices JSON: {e}"}))
        sys.exit(1)
    ...
    # Answer QCM
    result = answer_qcm(rms_energies, args.question, choices)
